...
type rootFlags struct {
	...
	// invocation. Mirrors the OPENROUTER_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		...
		Short: `Openrouter CLI — The only OpenRouter CLI built for fleet operations: budgets, anomaly alarms, and failover maps over a local usage…`,
		Long: `Openrouter CLI — The only OpenRouter CLI built for fleet operations: budgets, anomaly alarms, and failover maps over a local usage ledger — not another chat wrapper.
...
See README.md or the bundled SKILL.md for recipes.`,
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select accuracy,accuracy_stddev,agentic_index)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'openrouter-pp-cli profile list')")
	...
	rootCmd.AddCommand(newByokCmd(flags))
	rootCmd.AddCommand(newContainersCmd(flags))
	...
	rootCmd.AddCommand(newGuardrailsCmd(flags))
	...
	rootCmd.AddCommand(newOpenrouterAnalyticsCmd(flags))
	rootCmd.AddCommand(newOpenrouterAuthCmd(flags))
	rootCmd.AddCommand(newPresetsCmd(flags))
	rootCmd.AddCommand(newScimCmd(flags))
	...
	rootCmd.AddCommand(newBenchmarksPromotedCmd(flags))
	rootCmd.AddCommand(newChatPromotedCmd(flags))
	rootCmd.AddCommand(newClassificationsPromotedCmd(flags))
	...
	rootCmd.AddCommand(newNovelLimitsCmd(flags))
