# Verto Online Printed CLI Agent Guide

This directory is a generated `verto-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
verto-pp-cli doctor --json
verto-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
verto-pp-cli which "<capability>" --json
verto-pp-cli <command> --help
```
...
```bash
verto-pp-cli <command> --help
verto-pp-cli <command> --dry-run --agent
```
