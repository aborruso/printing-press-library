...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "verto-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("VERTO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("VERTO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "verto-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/verto-pp-cli/feedback.jsonl.
When ` + "`VERTO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`VERTO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  verto-pp-cli feedback list
  verto-pp-cli feedback list --limit 5
  verto-pp-cli feedback list --json`,
