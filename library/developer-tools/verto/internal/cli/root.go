...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "verto-pp-cli",
		Short: `Official IGM coordinate transforms between every Italian reference system, from the command line, with zero grid-file setup.`,
		Long: `Official IGM coordinate transforms between every Italian reference system, from the command line, with zero grid-file setup.
...
Health check: run 'verto-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("verto-pp-cli {{ .Version }}\n")

	rootCmd.PersistentFlags().BoolVar(&flags.asJSON, "json", false, "Output as JSON")
	rootCmd.PersistentFlags().BoolVar(&flags.jsonl, "jsonl", false, "Output as JSON Lines / NDJSON (one compact object per line)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'verto-pp-cli profile list')")
	...
	rootCmd.AddCommand(newNovelDetectCmd(flags))
	rootCmd.AddCommand(newNovelInspectCmd(flags))
	rootCmd.AddCommand(newNovelRoundtripCmd(flags))
	rootCmd.AddCommand(newNovelTargetsCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newSystemsPromotedCmd(flags))
	rootCmd.AddCommand(newConvertCmd(flags))
	rootCmd.AddCommand(newBatchCmd(flags))
	rootCmd.AddCommand(newGeojsonCmd(flags))
