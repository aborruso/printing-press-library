...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your API credentials."+
			"\n      Run 'verto-pp-cli doctor' to check auth status.", err))
	case strings.Contains(msg, "HTTP 403"):
		return authErr(fmt.Errorf("%w\nhint: permission denied. This API is configured without credentials, so the service may be blocking the request by rate limit, geography, bot protection, or endpoint policy."+
			"\n      Run 'verto-pp-cli doctor' to check connectivity.", err))
...
func printOutputWithFlags(w io.Writer, data json.RawMessage, flags *rootFlags) error {
	...
	// --jsonl: newline-delimited JSON (one object per line). For an array,
	// each element is emitted compact on its own line; a single object is
	// emitted compact on one line. Ideal for streaming into jq/pipelines.
	if flags.jsonl {
...
// printJSONL writes data as JSON Lines (NDJSON). Arrays expand to one element
// per line; any other value is written compact on a single line. Key order is
// preserved via json.Compact (no re-marshal).
func printJSONL(w io.Writer, data json.RawMessage) error {
	var arr []json.RawMessage
	if err := json.Unmarshal(data, &arr); err == nil {
		for _, el := range arr {
			line, err := compactJSONLine(el)
			if err != nil {
				return err
			}
...
func compactJSONLine(raw json.RawMessage) (string, error) {
...
func wantsHumanTable(w io.Writer, flags *rootFlags) bool {
	if flags.asJSON || flags.jsonl || flags.csv || flags.compact || flags.quiet || flags.plain {
		return false
	}
	if flags.selectFields != "" {
		return false
