...
func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://igmi.esercito.difesa.it/porta-magna/wps",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("VERTO_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "verto-pp-cli", "config.json")
	}
	cfg.Path = path
	...
	if v := os.Getenv("VERTO_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
