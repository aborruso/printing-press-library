...
import (
	...
	"aol-puglia-pp-cli/internal/client"
	"aol-puglia-pp-cli/internal/cliutil"
	"aol-puglia-pp-cli/internal/config"
	"aol-puglia-pp-cli/internal/store"
...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  aol-puglia-pp-cli doctor
  aol-puglia-pp-cli doctor --json
  aol-puglia-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					// No manual token — auto-fetch OAuth2 client_credentials is the default
					authConfigured = true
					report["auth"] = "auto-oauth2"
					report["auth_source"] = "auto:client_credentials"
				} else {
					authConfigured = true
			...
			if os.Getenv("AOL_PUGLIA_BEARER_AUTH") != "" {
				authEnvSet = append(authEnvSet, "AOL_PUGLIA_BEARER_AUTH")
			} else {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "auto:client_credentials"
				}
				authEnvInfo = append(authEnvInfo, "using "+authSource+" (no manual token needed)")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "aol-puglia-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("aol-puglia-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'aol-puglia-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'aol-puglia-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'aol-puglia-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'aol-puglia-pp-cli sync' to refresh."
	}
	return report
}
