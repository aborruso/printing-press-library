...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "aol-puglia-pp-cli", "http")
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "aol-puglia-pp-cli/1.0.0")
		}
...
func (c *Client) authHeader(ctx context.Context) (string, error) {
	...
	// Auto-fetch public OAuth2 client_credentials token when no token is configured.
	if authHeader == "" {
		token, err := getAutoToken()
		if err != nil {
			return "", fmt.Errorf("auto-fetching OAuth2 token: %w", err)
		}
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export AOL_PUGLIA_BEARER_AUTH=<your-token> or aol-puglia-pp-cli auth set-token <token>")
}
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.AolPugliaBearerAuth)
	}
