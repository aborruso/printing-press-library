...
- **`analytics --group-by oratore`** — Classifica gli oratori più attivi in Aula per numero di sedute in cui sono intervenuti. Gira **in diretta** sul backend `/bd/resoconti` (una richiesta per deputato della legislatura), quindi richiede `--legisl` e impiega ~1 minuto. Es: `analytics --type resoconti --group-by oratore --legisl 18`.
