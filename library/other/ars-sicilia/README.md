...
- **`--testo` is AND over the whole document; `--frase` matches a phrase** ℹ️: `--testo "aree idonee"` builds `(aree E idonee)`, so both words must appear *somewhere* in the document — on a text as long as a bill that also matches acts with one word in article 3 and the other in article 40 (peschicoltura, coworking). `--frase "aree idonee"` builds `(aree adj idonee)`: adjacent, in the given order, which surfaces the acts that actually legislate on the topic (ddl 803, 726). A single word passes through unchanged, and a value containing parentheses — or an operator written in **uppercase**, as in `--frase "aree E idonee"` — is left verbatim, provided the phrase also has lowercase in it: in an all-caps title copy-pasted from the portal, "SVILUPPO E COESIONE", the case carries no signal and the conjunction is treated as a word. A **lowercase** Italian conjunction inside a phrase is a word, not an operator: `--frase "coesione e crescita"` (the title of a budget bill) used to be shipped verbatim and read by the portal as a plain AND, silently. It now drops the conjunction and widens the distance to match — `(coesione adj3 crescita)` — and says so on stderr, in the envelope and in `--dry-run`, because proximity is not the exact locution the flag promises. A dropped conjunction is worth **two** positions, not one: in an Italian title it brings along the article nobody types when searching. Bill 1200 is titled "Disposizioni per **la** coesione e **la** crescita", three words between the two searched terms, so `adj2` returned nothing for the name the press uses for that budget — measured on the portal on 2026-09-05: `coesione adj2 crescita` → 0 rows, `adj3` → bill 1200 in first position. The cost in precision is measured too: on ddl 969 "prevenzione e contrasto" in legislature 18 the AND returns 144 rows, `adj2` 41 and `adj3` 170, and the bill is in all three, while strict adjacency over the surviving words returns 3 and misses it — the portal indexes the conjunction as a position. When the widened distance still returns nothing, the hint names `--testo` (same words, no order constraint) before `--isis-query`. Only "e" and "o" are dropped: the vocabulary also contains ordinary Italian words - "seguito", "vicino", "meno", "no", "escluso" - and removing one of those does not soften the search, it falsifies it (`--frase "aree meno idonee"` would become "aree idonee", the opposite), so there the phrase is shipped as it was and the warning says the locution cannot be expressed instead of staying silent. For a hand-written expression use `--isis-query`. The flag does not exist on `resoconti`, `sommari` and `convocazioni` (the `/bd/` backend takes no ISIS expression) — there the text search is `--testo`.
- **Full-text results are not ranked by relevance, and a short list is not proof of absence** ⚠️: the portal returns matches in its own order (roughly newest first), not by how well they match `--testo`. The CLI now pulls the rows whose **title** contains every search term to the front, which is usually enough: `ddl cerca --legisl 17 --testo "gestione rifiuti" --limit 100` moves ddl 290 ("Riforma degli ambiti territoriali ottimali e nuove disposizioni per la gestione integrata dei rifiuti") from row 75 to row 2. **That reordering only sorts the window you already downloaded** — it cannot surface a document that sits past `--limit`. So when none of the rows shown has the terms in its title, a stderr hint says exactly that: read it as "the act you want is probably further down", raise `--limit`, or use `--frase` for the exact phrase. On `resoconti`, `sommari` and `convocazioni` the hint stops at `--limit`, since `--frase` does not exist on the `/bd/` backend. Whenever the result set is cut short, `*/cerca` says so too — treat both hints as "widen or narrow before concluding it does not exist". On `ddl`, a cut free-text search adds one more hint: when the story you start from gives the date of a sitting but not the act's number, the committee summary of that day names the bills by number — `commissioni sommari --legisl 18 --data 2024-09-17` returns that day's 7 sittings and names ddl 780 among them (`--commissione` only narrows further; a single day is small enough that the `/bd/` size truncation does not bite), which `ddl cerca --anno 2024 --frase "enti locali"` leaves outside its window.
...
- **Iter events are read two different ways when sitting and date do not line up** ⚠️: on `ddl iter` and `legge cronologia` alike, and only one of the two is a contradiction. **Same date, different Aula sitting numbers**: the Assembly holds one sitting a day, so at least one number is wrong and the source does not say which — those events carry `anomalia: true` and keep the act's own page as their `url`, because a link built on a contested number could show the wrong document. You resolve them **from the date**, the reliable half (`resoconti cerca --legisl 17 --data 2020-02-19`). **Same sitting number, several dates**: that is a sitting opened one day, suspended and resumed until the text passes — how the Assembly does budgets — not a contradiction. The reading is **verified against the resoconti archive** before it is asserted: one request per candidate sitting, only in the iters where a number appears on several dates. Confirmed, those events carry `seduta_pluri_giorno: true` and, on the resumption days, `data_apertura` — the archive's own opening date, in ISO — and keep the resoconto link. A number the archive does not have, or opens on a day the iter never declares, is not a resumed sitting but a wrong number: it stays `anomalia: true` without a link. If the archive does not answer, the reading is kept and the note says it is unverified, rather than flipping the marker with connection quality. Measured on four budget laws across two legislatures: L.R. 1/2019 (sitting 98, opened 31 January, final vote 15 February), L.R. 9/2020 (187, 28 April → 2 May), L.R. 28/2024 (142, 5 → 7 November), L.R. 1/2025 (147, 18 → 28 December) — in all four the number is right and the record exists, filed under the opening day. That is why searching the archive by the date of the vote finds nothing (`resoconti cerca --legisl 17 --data 2020-05-02` → `[]`) while `resoconti get 17 187` returns it: the reason lands both on stderr and in the report's `note` field, which `--select` cannot strip. Before concluding a resoconto is missing, read those two fields: neither says the archive has a hole.
...
- **New: punctuation inside a search value no longer fails the search** ⚠️ *(behaviour change)*: the ISIS engine does not ignore punctuation inside a filter value — it refuses the query. Measured on 2026-09-06 against the `ddl` archive, one character at a time: `' " , - / . ; : + * ? & ! ( = #` all return the engine's error page («Impossibile creare la Query»); letters, digits, space, the `$` truncation operator and `%` go through. That refused `--iter "Approvato dall'Assemblea"` (the state name the portal itself writes), `--firmatario "D'Agostino"` (a Sicilian surname), `--testo "dell'ambiente"`, `--materia "sanita'"` and `--testo "COVID-19"` — and it surfaced as the *threshold* message, sending you to narrow a period that was never the problem. The CLI now cleans the value before sending and says on stderr what actually left (`--firmatario «D'Agostino» è partito come «D Agostino»`). Replacing punctuation with a space is faithful to the index, not a fallback: the portal tokenises on punctuation, and a space inside a field value means adjacency — `--iter "Approvato dall Assemblea"` returns exactly the 16 rows of `--iter "Assemblea"` for 2026, while `--iter "Assegnato Assemblea"` (two real states, not adjacent) returns none. Values containing parentheses, `--isis-query`, and the date/number forms the CLI builds itself (`260101/261231`, `2026-07-01:2026-07-31`) are left verbatim. `--isbn` is the exception and it is not deducible from the value's shape: two numeric fields of the same archive behave oppositely — on `--dewey "340.5"` the dot really does separate two tokens, while an ISBN's punctuation formats a single number. The archive is not consistent with itself either (`9788875241667` as one token, another record as `978 88 98231-25-6`), and neither spelling alone covers it, so `--isbn` sends **both in OR**: `--isbn "978-88-7524-166-7"` leaves as `(9788875241667 O (978 88 7524 166 7)).ISBN`. **The refusals are now distinguished**: `(QR997)` is the threshold and narrowing the period works; `Impossibile creare la Query` (no code) and `(QR999) Operando con crt non validi` are syntax, where narrowing changes nothing — the CLI stops wasting the sub-period splitting on them. The second one carries a `QRxxx` code just like the threshold, so telling them apart by the presence of a code is not enough.
- **`ddl cerca --iter` filters on states the bill has *traversed*, not its current one** ⚠️: the field indexes the whole history, so `--iter "Assegnato"` also returns bills long past assignment (bill 779, presented in 2024, comes back from `--iter "Assegnato" --anno 2024` while being much further along). For one bill's current state use `ddl iter <legisl> <numero>`. With that in mind the filter answers set-level questions that had no route before: `--legisl 18 --iter "Approvato dall'Assemblea"` → 112 bills, `--iter "Assegnato per esame Commissione PRIMA"` → 52. Two caveats: the value is a **phrase** and must be written as the portal writes it in the bill's iter (a space means adjacency, and an invented term returns `[]`, indistinguishable from "no such case"); and the source's index **lags on recent bills** — bill 1196, assigned on 2026-08-05, was still absent from `--iter "Assegnato" --anno 2026` on 2026-09-06. Cross-check recent acts with `ddl iter`.
- **`--select` with names that exist nowhere is now ignored for real** ⚠️ *(behaviour change)*: the stderr hint always said «will be ignored», but the projection still ran and every nested array came out as empty objects — `ddl get 18 779 --select iter` printed `firmatari: [{}, {}]`, and `--select pippo` did the same to `eventi` on `ddl iter`, `atti` on `deputato profilo`, every `sezioni[].risultati` of `commissione dossier` and every row of a `cerca`, with or without `--envelope`. Now when **none** of the requested names resolves, the full payload is printed and the hint tells you the right names (`iter` lives under `fields.Iter`). A mixed selector (`numero,pippo`) keeps filtering on the names that exist, as before.
- **An empty search whose `--data` window reaches the last 45 days says it may be source latency** ℹ️: `interrogazioni cerca --data 2026-08-15:2026-09-04` used to return `[]` and nothing else, while the archive stopped a month earlier — the gap and the coverage lived in two different commands. The hint (stderr, and inside `--envelope`) now says the portal publishes 9-45 days late and points to `sync coverage --resources <archive>` for the real last date. No extra request is made; a window that ends months or years ago, or starts in the future, stays silent, because there the empty result is an empty result; a window straddling today is judged as if it ended today.
...
- **`ddl iter`** — Ricostruisce la cronologia completa di un disegno di legge: presentazione, passaggio in commissione, lavori d'aula, eventuale promulgazione come legge regionale.
  ...
  **`fase` dice dove l'evento è avvenuto, non dove il testo è diretto**: il verbo nomina la destinazione, la seduta dichiara il luogo, e dove le due si contraddicono vince la seduta. «Esitato per Aula» è l'esito del lavoro di commissione e la riga dichiara una seduta di commissione, quindi la fase è `commissione`: prima il verbo bastava a farne un evento d'Aula, e chi filtrava `fase == "aula"` per il voto finale si portava dentro una riga di commissione con la data sbagliata di settimane (sul ddl 5030 il 16 giugno invece dell'8 luglio). Vale anche al contrario: «Rinviato Commissione 0400 Seduta n. 255 AULA» è il rinvio in commissione deciso **in Aula** (la 255 è il resoconto d'Aula del 10 giugno 2026, ed è quello che la CLI linka), quindi la fase è `aula` e la commissione di destinazione resta in `sede`. Sul ddl 6030 quella riga usciva come `commissione` e un filtro per fase perdeva un passaggio d'Aula. Le altre fasi non si toccano: dove il verbo non nomina un luogo non c'è contraddizione da sciogliere.

  Il campo **`sede`** dà la commissione in forma canonica (l'ordinale, quello che gli altri comandi accettano) **sulle righe in cui il portale dichiara una seduta**: è lì accanto che la scrive, e la si legge da lì anche quando il verbo dell'evento la nomina altrimenti o non la nomina. Le commissioni speciali tengono il nome per esteso, e il nome d'uso resta in `titolo`, che è verbatim — «Parere Commissione Bilancio» ha `sede: Commissione SECONDA`. Sulle righe senza seduta (assegnazioni, invii) vale la dicitura del verbo, quindi la stessa commissione può apparire con due nomi nella stessa cronologia: non raggruppare per `sede` dandola per canonica. Nella sede la CLI ricompone la parola che la fonte spezza con un trattino e un a-capo (l'HTML della scheda del ddl 5030 scrive `Commissione</a> T-<br> ERZA`, e l'iter usciva con due sedi dove ce n'è una); il testo dell'evento resta invece verbatim in `titolo`, garbled compreso.
...
- **`deputato profilo`** — Aggrega in un'unica vista tutti gli atti firmati o pronunciati da un deputato: DDL, interrogazioni, interpellanze, mozioni, ordini del giorno, risoluzioni e interventi in resoconti d'aula. `--data` (range `YYYY-MM-DD:YYYY-MM-DD`) filtra per data di presentazione/seduta su tutti i sotto-archivi, per query storiche mirate senza dover alzare `--limit`.
  ...
  ```
- **`commissione dossier`** — Vista completa su una commissione: convocazioni in calendario, sommari lavori, DDL assegnati e pareri richiesti al Governo regionale. Accetta il codice `1`-`6`, l'ordinale (`PRIMA`..`SESTA`) o un frammento della denominazione d'archivio. Le **commissioni speciali** (Antimafia, Statuto, Unione Europea) non hanno un codice e si raggiungono solo per denominazione, che non coincide con l'etichetta d'uso corrente: `"Antimafia"` non corrisponde a nulla, la denominazione è *«Commissione d'inchiesta e vigilanza sul fenomeno della mafia e della corruzione in Sicilia»*. Un termine che non aggancia nessuna commissione non produce un dossier vuoto: l'errore elenca le denominazioni della legislatura. **`conteggio` è quanto è stato scaricato, `totale` quanti ce ne sono**: le sezioni servite dall'ISIS portano ora il totale reale (sulla PRIMA della XVIII, `pareri` 82 e `ddl_assegnati` 637) e `troncato` compare solo quando `totale > conteggio`. Il totale di `ddl_assegnati` è quello di una ricerca testuale sull'ordinale, non dell'elenco degli assegnati — l'archivio dei ddl non espone l'assegnazione come campo, e `note` lo dichiara; `convocazioni` e `sommari` restano senza totale perché il backend `/bd/` non lo pubblica.
