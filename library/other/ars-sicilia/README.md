...
- **`legge cronologia` needs `--anno` when the number repeats** ⚠️: the same law number recurs in different years of one legislature — the XVIII has two L.R. 26 (7 Oct 2024, 10 Jun 2025). The archive returns one row and the command takes it, so without `--anno` you can get a perfectly coherent timeline for the wrong act. A stderr hint names the law it picked (`uso la L.R. 26 promulgata il 7.10.2024`): check that date, or pin `--anno`. The report's root carries `ddl_originari`, the numbers of the bills the law came from (more than one when they were merged) — the direct handle for `ddl iter`.
