...
- **An empty search whose `--data` window reaches the last 45 days says it may be source latency** ℹ️: `interrogazioni cerca --data 2026-08-15:2026-09-04` used to return `[]` and nothing else, while the archive stopped a month earlier — the gap and the coverage lived in two different commands. The hint (stderr, and inside `--envelope`) now says the portal publishes 9-45 days late and points to `sync coverage --resources <archive>` for the real last date. No extra request is made; a window that ends months or years ago stays silent, because there the empty result is an empty result.
