...
- **`--testo` is AND over the whole document; `--frase` matches a phrase** ℹ️: `--testo "aree idonee"` builds `(aree E idonee)`, so both words must appear *somewhere* in the document — on a text as long as a bill that also matches acts with one word in article 3 and the other in article 40 (peschicoltura, coworking). `--frase "aree idonee"` builds `(aree adj idonee)`: adjacent, in the given order, which surfaces the acts that actually legislate on the topic (ddl 803, 726). A single word passes through unchanged, and a value containing parentheses — or an operator written in **uppercase**, as in `--frase "aree E idonee"` — is left verbatim, provided the phrase also has lowercase in it: in an all-caps title copy-pasted from the portal, "SVILUPPO E COESIONE", the case carries no signal and the conjunction is treated as a word. A **lowercase** Italian conjunction inside a phrase is a word, not an operator: `--frase "coesione e crescita"` (the title of a budget bill) used to be shipped verbatim and read by the portal as a plain AND, silently. It now drops the conjunction and widens the distance to match — `(coesione adj2 crescita)` — and says so on stderr, in the envelope and in `--dry-run`, because proximity is not the exact locution the flag promises. Measured on ddl 969 "prevenzione e contrasto" in legislature 18: the AND returns 144 rows, `adj2` returns 41 and includes it, while strict adjacency over the surviving words returns 3 and misses it — the portal indexes the conjunction as a position. Only "e" and "o" are dropped: the vocabulary also contains ordinary Italian words - "seguito", "vicino", "meno", "no", "escluso" - and removing one of those does not soften the search, it falsifies it (`--frase "aree meno idonee"` would become "aree idonee", the opposite), so there the phrase is shipped as it was and the warning says the locution cannot be expressed instead of staying silent. For a hand-written expression use `--isis-query`. The flag does not exist on `resoconti`, `sommari` and `convocazioni` (the `/bd/` backend takes no ISIS expression) — there the text search is `--testo`.
...
- **`ddl iter`** — Ricostruisce la cronologia completa di un disegno di legge: presentazione, passaggio in commissione, lavori d'aula, eventuale promulgazione come legge regionale.
  ...
  **`fase` dice dove l'evento è avvenuto, non dove il testo è diretto**: «Esitato per Aula» è l'esito del lavoro di commissione, e la riga dichiara una seduta di commissione, quindi la fase è `commissione`. Prima il verbo bastava a farne un evento d'Aula, e chi filtrava `fase == "aula"` per il voto finale si portava dentro una riga di commissione con la data sbagliata di settimane (sul ddl 5030 il 16 giugno invece dell'8 luglio). Il criterio è la seduta dichiarata dal portale: le righe d'Aula portano il marcatore AULA al posto del nome della commissione.
